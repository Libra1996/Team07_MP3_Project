...
typedef struct {
  ...
  /*                             SLAVE EXTRA VARIABLES                            */
  bool is_register_address_or_data;
...
// LCD function
bool i2c__write_LCD(i2c_e i2c_number, uint8_t slave_address, uint8_t data) {
  uint8_t empty;
  return i2c__write_slave_data(i2c_number, slave_address, data, &empty, -1);
}
...
static bool i2c__handle_state_machine(i2c_s *i2c) {
  ...
  // Slave acknowledged its address, so send the first register 0x18
  case I2C__STATE_MT_SLAVE_ADDR_ACK:
    ...
    } else if (-1 == i2c->number_of_bytes_to_transfer) { // LCD
  ...
  // Slave acknowledged the data byte we sent, so send more bytes or finish the transaction by sending STOP 0x28
  ...
  case I2C__STATE_ARBRITRATION_LOST:
    ...
    //   i2c->is_register_address_or_data = true; // setting up to get register address
    //   i2c__set_ack_flag(lpc_i2c);
    //   i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    //   break;
    // case I2C__STATE_SLAVE_RECEIVER_MASTER_TRANSMITTER_ACK_SENT_0X80:
    //   if (i2c->is_register_address_or_data) {
    //     i2c->register_address_of_slave = lpc_i2c->DAT;
    //     i2c->is_register_address_or_data = false; // done with reg address
    //   } else {
    //     if (i2c_slave_callback__write_memory(i2c->register_address_of_slave++, lpc_i2c->DAT)) {
    //       ;
    //     } else {
    ...
    //   if (i2c_slave_callback__read_memory(i2c->register_address_of_slave++, &lpc_i2c->DAT)) {
    //     ;
    //   } else {
